...
void uart_transmit_output(unsigned char value)
{
    ...
    extern void  Send_Data2(unsigned char value);
     Send_Data2(value);	
/*
    //Example:
...
void all_data_update(void)
{
  ...
  // mcu_dp_enum_update(DPID_TEMP_UNIT_CONVERT,"C"); //枚举型数据上报;
   mcu_dp_value_update(DPID_MAXTEMP_SET,28); //VALUE型数据上报;
   mcu_dp_value_update(DPID_MAXHUM_SET,28); //VALUE型数据上报;
    mcu_dp_value_update(DPID_MINIHUM_SET,18); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_TEMP_ALARM,1); //枚举型数据上报;
    mcu_dp_enum_update(DPID_HUM_ALARM,0); //枚举型数据上报;
    mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,20); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,20); //VALUE型数据上报;
   mcu_dp_value_update(DPID_TEMP_SENSITIVITY,5); //VALUE型数据上报;
  mcu_dp_value_update(DPID_HUM_SENSITIVITY,5); //VALUE型数据上报;
   mcu_dp_bool_update(DPID_SWITCH,1); //BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_SET,20); //VALUE型数据上报;
