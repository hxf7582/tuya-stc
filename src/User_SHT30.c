...
//	int AT;
  ...
  result[0] = RecvData();  //读到的数据暂存在result数组里
	...
	if((sht_crc(result, 2)==result[2]) && (sht_crc(result+3, 2)==result[5]))//crc校验
	{
		...
		T = FlashBuffer.temper/10; 
		TG = FlashBuffer.temper%10; 
		//温度计算
    ...
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,FlashBuffer.humidity); //VALUE型数据上报;
		OLED_ShowNum(70,4,T,2,16);   //显示温度
		
			
		OLED_ShowNum(94,4,TG,1,16);   //显示温度
		
		if(T>0)
			//OLED_ShowCHinese(102,4,6);
			OLED_ShowChar(102,4,'`',16);
		else
			OLED_ShowCHinese(92,4,6);
			//OLED_ShowChar(92,4,'`',16);
		
		OLED_ShowNum(70,6,FlashBuffer.humidity,2,16);  //显示湿度
	...
	{
		OLED_ShowStr(70,4,"ERROR",16);  //显示错误
	}
}